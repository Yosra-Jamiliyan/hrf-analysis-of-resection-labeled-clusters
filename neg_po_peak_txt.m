% Revised Script for Type 1 Delay and Dispersion Analysis with updated FWHM and peak classification
...
partially_discordant = [];
fully_discordant = [1,2,3,4,5];
...
for i = 1:n_clusters
    pe1 = niftiread(fullfile(path, sprintf('pe1_cluster%d.nii.gz', i)));
    pe2 = niftiread(fullfile(path, sprintf('pe2_cluster%d.nii.gz', i)));
    pe3 = niftiread(fullfile(path, sprintf('pe3_cluster%d.nii.gz', i)));
    mask = pe1 ~= 0;
    n_voxels = nnz(mask);
    ...
    for x = 1:size(pe1, 1)
        for y = 1:size(pe1, 2)
            for z = 1:size(pe1, 3)
                if mask(x, y, z)
                    beta = [pe1(x, y, z), pe2(x, y, z), pe3(x, y, z)];
...
for g = 1:numel(group_names)
    ...
    writematrix(output, filename, 'Delimiter', ' ');
end
