% Revised Script for Type 2 Delay and Dispersion Analysis with updated FWHM and peak classification
...
for i = 1:n_clusters
    pe4 = niftiread(fullfile(path, sprintf('pe4_cluster%d.nii.gz', i)));
    pe5 = niftiread(fullfile(path, sprintf('pe5_cluster%d.nii.gz', i)));
    pe6 = niftiread(fullfile(path, sprintf('pe6_cluster%d.nii.gz', i)));
    mask = pe4 ~= 0;
    n_voxels = nnz(mask);
    ...
    for x = 1:size(pe4, 1)
        for y = 1:size(pe4, 2)
            for z = 1:size(pe4, 3)
                if mask(x, y, z)
                    beta = [pe4(x, y, z), pe5(x, y, z), pe6(x, y, z)];
